...
void avdc_access(avdark_cache_t * self, avdc_pa_t pa, avdc_access_type_t type)
{
	...
	for (offset = 0; offset < self->assoc; offset++) {
		hit = self->lines[index][offset].valid && self->lines[index][offset].tag == tag;
		if (hit)
			break;
	}

	if (!hit) {
		if (self->assoc == DIRECT_MAPPED) {
			...
			for (offset = 0; offset < self->assoc; offset++) {
				if (!self->lines[index][offset].lru) {
					self->lines[index][offset].valid = 1;
					self->lines[index][offset].tag = tag;
					self->lines[index][offset].lru = 1;
					break;
				}
			}
		}
	...
	switch (type) {
		case AVDC_READ:	/* Read accesses */
			...
			if (self->assoc != DIRECT_MAPPED) {
				self->lines[index][(offset + 1) % 2].lru = 0;
				self->lines[index][offset].lru = 1;
			}
...
{
	/* TODO: Update this function */
	for (int offset = 0; offset < self->assoc; offset++) {
		for (int i = 0; i < self->number_of_sets; i++) {
			self->lines[i][offset].valid = 0;
			self->lines[i][offset].tag = 0;
			self->lines[i][offset].lru = 0;
		}
	}
}
...
int avdc_resize(avdark_cache_t * self, avdc_size_t size, avdc_block_size_t block_size, avdc_assoc_t assoc)
{
	...
	if (!is_power_of_two(size) || !is_power_of_two(block_size) || !is_power_of_two(assoc)) {
...
void avdc_print_internals(avdark_cache_t * self)
{
	...
	for (i = 0; i < self->number_of_sets; i++)
		for (offset = 0; offset < self->assoc; offset++)
			fprintf(stderr, "tag: <0x%.16lx> valid: %d\n", (long unsigned int)self->lines[i][offset].tag, self->lines[i][offset].valid);
}
